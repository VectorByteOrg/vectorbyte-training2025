...
title: "Introduction to Time Series Methods"
...
It turns out that we fiddle with a non-stationary TS to get something that is stationary through ***`r myred("differencing")`***:

- computing the differences between consecutive data points (or at another fixed lag).
...
Temps<-airquality$Temp
par(mfrow=c(2,2))
plot(t, Temps, type="l", lwd=2, 
     ylab="Temps", xlab="Day", 
     main="Temperature")
acf(Temps)
plot(diff(Temps), type="l", lwd=2, 
     ylab="diff(Temps)", xlab="Day", 
     main="diff(Temperature)")
acf(diff(Temps))
...
par(mfrow=c(2,2))
plot(ld, type="l", lwd=2, 
     ylab="deaths", xlab="Months", 
     main="UK deaths")
acf(ld)
plot(diff(ld, lag=12), type="l", lwd=2, 
     ylab="diff(deaths, lag=12)", xlab="Months", 
     main="differenced UK deaths")
acf(diff(ld))
...
$$
\begin{align*}
y'_t =~& \beta_0 + \beta_1 y'_{t-1} + \cdots + \beta_p y'_{t-p} \\
& + \varepsilon_t + \theta_1 \varepsilon_{t-1} + \cdots \theta_q \varepsilon_{t-q} 
\end{align*}
$$
That is, it's an ARMA model but on the ***`r myblue("differenced")`*** time series to take care of trends.
...
plot(m.ld$residuals)
acf(m.ld$residuals, main="")
hist(m.ld$residuals, main="")
```
...
plot(ld, xlab="month", ylab="deaths", type="l", 
     col=1, lwd=2)
lines(m.ld$fitted, col=2, lty=2, lwd=2)
legend("topright", legend=c("data", "fit"), col=1:2, lty=1:2, lwd=2)
```

## Forecasting 

```{r, echo=TRUE}
## forecasting at a 12 mo horizon
f.ld<-forecast::forecast(m.ld, h=12)
```

```{r, fig.align='center', fig.width=6, fig.height=5}
plot(ld, xlab="month", ylab="deaths", type="l", 
     col=1, lwd=2, 
     xlim=c(0, 84), ylim=c(600, 4000))
lines(m.ld$fitted, col=2, lty=2, lwd=2)
lines(72+1:12, f.ld$mean, col="dodgerblue", lwd=2)
lines(72+1:12, f.ld$lower[,2], col="dodgerblue", lwd=2, lty=3)
lines(72+1:12, f.ld$upper[,2], col="dodgerblue", lwd=2, lty=3)

legend("topleft", legend=c("data", "fit", "FC"), col=c(1:2, "dodgerblue"), lty=c(1,2,1), lwd=2)
```


## Elaborating on ARIMA

You can include multiple sets of differences in your model. 

`r sk1()`

Specifically, if your differencing reflects the number of time steps over which you observe a periodic pattern, you can add this extra bit on top and create a `r myred("SARIMA")` model.
...
## Fun trick -- AR model of residuals
...
Often you find that your predictions/forecasts aren't great because you don't account for autocorrelation in the ***`r mygrn("error")`*** 

- if you were higher than expected at time $t-1$ you're likely to still be a bit high at time $t$. 


`r myblue("How can we incorporate this?")`
...
1. Fit the model you want
2. Fit an AR (or an ARIMA!) model to the residuals
3. Make the prediction/forecast from these bits together

`r sk1()`

This works surprisingly well -- we'll see an example of this in the practical. 

## ARIMA vs. regression approaches
...
We're going to practice our TS modeling approaches through the lens of `r myred("forecasting")`. 
...
5. `r myblue("Partitioning")` - need to test our forecasts
6. Modeling - what methods? what predictors?
7. `r myblue("Evaluation")` - how good is the forecast?
8. Implementation - make it repeatable, maybe automate

## Evaluating predictions and forecasts

We've already learned a few ways to explore how good our model is on the data that we've trained it on (within sample). 

Mostly we've focused on *`r myblue("qualitative metrics")`*:

- Does the pattern look like the data?
- Are there patterns left in the residuals?
- Are the residuals still auto-correlated?
- Are the residuals normally distributed?

What about `r myred("quantitative metrics")`?


## Root Mean Squared Error (RMSE)

RMSE is one of the simplest quantitative metrics to calculate:
$$
\text{RMSE} = \left(\frac{1}{n}\sum_{i=1}^n (y_i - \hat{y}_i)^2 \right)^{1/2}
$$
where $e_i = y_i - \hat{y}_i$ is the residual for the $i^{\text{th}}$ data point. 

`r sk1()`

This gives us a quantitative measure of how close, overall, our predictions are to the data. 

## In-sample vs. Forecasting error
...
`r myred("What about forecasting?")`
...
Data partitioning -- dividing data into a **`r mygrn("training")`** set to fit the model and a **`r myblue("testing")`** set where we make predictions -- is a standard statistical technique. 
...
However, ***`r myred("we cannot remove data at random from a time series")`*** as it will impact all aspects of the analysis. 

## Data Partitioning for Forecasting

When we are dealing with time dependent data, we instead `r myred("systematically")` leave out data. 

If we have data from times 1 to $t_{\text{max}}$ we choose a final window of data to leave out:

- `r mygrn("training")`: times from 1 to $t_{\text{test}}$
- `r myblue("testing")`: times from $t_{\text{test}}+1$ to $t_{\text{max}}$

Then we fit the model just to the `r mygrn("training")` data, make the ***forecast*** from the model, and calculate RMSE just on the `r myblue("testing")` data.

## Time Series and Forecasting Practical
...
In the next practical, we're going to use our new time series methods on a new data set on `r mygrn("tree greenness")`. 
